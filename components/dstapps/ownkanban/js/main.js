...
class Elements {
  ...
  static divStopperFinished = '<svg id="stopper-svg" class="stopper-finished" style="margin-top: -4px;" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="Layer_1" style="enable-background:new 0 0 612 792;" version="1.1" viewBox="0 0 612 792" xml:space="preserve"><style type="text/css">.st0{fill:#41AD49;}</style><title>Задача завершена</title><g><path class="st0" d="M562,396c0-141.4-114.6-256-256-256S50,254.6,50,396s114.6,256,256,256S562,537.4,562,396L562,396z    M501.7,296.3l-241,241l0,0l-17.2,17.2L110.3,421.3l58.8-58.8l74.5,74.5l199.4-199.4L501.7,296.3L501.7,296.3z"/></g></svg>'
  static divStopperStop = '<svg data-stopper-type="stop" id="stopper-svg" xmlns="http://www.w3.org/2000/svg" class="ionicon" viewBox="0 0 512 512"><title>Задача приостановлена(стоит на стопере)</title><path d="M448 256c0-106-86-192-192-192S64 150 64 256s86 192 192 192 192-86 192-192z" fill="indianred" stroke="currentColor" stroke-miterlimit="10" stroke-width="32"/><path d="M310.4 336H201.6a25.62 25.62 0 01-25.6-25.6V201.6a25.62 25.62 0 0125.6-25.6h108.8a25.62 25.62 0 0125.6 25.6v108.8a25.62 25.62 0 01-25.6 25.6z"/></svg>'
...
<div style="display: inline-block; vertical-align: middle;" class="loader-titles">
  ...
  static showStopperStart(eventTaskStopper, evtItem) {

    let taskId = evtItem.getAttribute("data-task-id");
    const result = Logic.taskList.filter(task => task.id == taskId);

    let task = result[0];

    console.log(result, 'filt');
    eventTaskStopper.innerHTML = Elements.divStopperStart;
    eventTaskStopper.onclick = Click.stopperClick.bind(this, taskId, eventTaskStopper, task);
  ...
  static showStopperStop(eventTaskStopper, evtItem) {
    ...
    console.log(result, 'filt');
    eventTaskStopper.innerHTML = Elements.divStopperStop;
    eventTaskStopper.onclick = Click.stopperClick.bind(this, taskId, eventTaskStopper, task);
    console.log(Elements.stopper);
  ...
  static showStopperFinished(eventTaskStopper) { //Зеленая галочка для завершенной задачи



    eventTaskStopper.onclick = null;
    eventTaskStopper.innerHTML = Elements.divStopperFinished;
    console.log(Elements.stopper);
...
BX.addCustomEvent("SidePanel.Slider:onOpenComplete", function (event) {
  ...
  matches = url.match(/\/task\/view\//i);
  console.log(matches);

  if (matches) {
    if (matches[0] == "/task/view/") {
...
BX.addCustomEvent("SidePanel.Slider:onMessage", function (event) {
  ...
  let sliderEventSwithcer = {
    'setStopper': function setStopper() {

      // console.log(event.data, "слайдер");

      let taskId = event.data.taskId;

      let divStopper = document.getElementById('stopper-' + taskId).innerHTML = Elements.divStopperStop; //По постановке стоппера рисую икноку стопа на карточке
      Logic.stopStartedTask(taskId); //нужен await

    },
    'unsetStopper': function unsetStopper() {

      
      let taskId = event.data.taskId;

      let divStopper = document.getElementById('stopper-' + taskId)
      divStopper.innerHTML = Elements.divStopperStart; //По снятию стоппера рисую икноку старта на карточке
      divStopper.classList.remove('cursor-initial');
      Logic.startStoppedTask(taskId); //нужен await

    },
  }

  sliderEventSwithcer[event.data.eventActivity]();
...
class Click {
  ...
  static additionalInfoClick = async function (task) {
    ...
    BX.SidePanel.Instance.open("https://dstural124.ru/local/dstapps/ownkanban/stoppercomments.php", {
      requestMethod: "post",
      requestParams: { // post-параметры
        mode: "showAdditionalInfo",
        userId: User.id,
        taskId: task.id,
  ...
  static stopperClick = async function (taskId, divStopper, task) {
    console.log(task);
    ...
    if (stopper.getAttribute("data-stopper-type") == 'start') {
      ...
      //Открываем окно дополнительной информации, содержащих комментарии к стопперу
      await BX.SidePanel.Instance.open("https://dstural124.ru/local/dstapps/ownkanban/stoppercomments.php", {
        requestMethod: "post",
        requestParams: { // post-параметры
          mode: "setStopper",
          userId: User.id,
      ...
      //Тут постановка стопера, дублируется в событии
      // divStopper.innerHTML = Elements.divStopperStop;
      // await Logic.stopStartedTask(taskId);
      ...
      await Logic.startStoppedTask(taskId);
...
class Logic {
  ...
  static async getAddiotionalTaskInfo() { //переделать -- модель получения дополнительных данных по задаче
    ...
    if (d.length == 0) {
      ...
      Logic.taskList.forEach((task, i) => {
        ...
        task.additionalDbInfo = null;
        // task.dbPriority = dbArr[0].priority;
        task.additionalDbInfo = dbArr[0];
        console.log(task)

        if (task.additionalDbInfo.stopper == 1) {
          let params = {
            "taskId": task.id,
            'fields': { STATUS: 2 }
          }
      
          let tasks = app.bitrixHook(params, "https://dstural24.ru/rest/830/l7bann8u7zjtvy8v/tasks.task.update.json"); //нужен await
        }
  ...
  static async stopStartedTask(taskId) {
    console.log('started');

    let params = { taskId: taskId, }
    let log = await app.bitrixHook(params, "https://dstural24.ru/rest/830/l7bann8u7zjtvy8v/tasks.task.pause.json");
    console.log('stopped', log);
  }

  static async startStoppedTask(taskId) {
    console.log('started');

    let params = { taskId: taskId}
    let log = await app.bitrixHook(params, "https://dstural24.ru/rest/830/l7bann8u7zjtvy8v/tasks.task.start.json");
    console.log('stopped', log);
  }
  ...
  static async updateMovedTask(project, evtTo, evtItem) {
    ...
    const eventTaskStopper = evtItem.getElementsByClassName('task-stopper')[0];
    console.log(evtItem);
    //Если перемещаем в последний колодец, то "завершаем задачу", если в любой другой колодец, то возобновляем
    if (stageIndex == 4) {
      //Так как колодец последний, рисуем галочку вместо стопера
      Elements.showStopperFinished(eventTaskStopper);
      await app.bitrixHook(params, "https://dstural24.ru/rest/830/l7bann8u7zjtvy8v/tasks.task.complete.json");
    } else {
      Elements.showStopperStart(eventTaskStopper, evtItem);
      await app.bitrixHook(params, "https://dstural24.ru/rest/830/l7bann8u7zjtvy8v/tasks.task.renew.json");
      await app.bitrixHook(params, "https://dstural24.ru/rest/830/l7bann8u7zjtvy8v/tasks.task.start.json");
    }
    ...
    if (stageIndex != 1) {
      ...
      //Тут функции показа стопера в зависимости от стадии
      ...
      // Draw.draw(); //ВООБЩЕ ЗАПРЕТИТЬ ВЗАИМОДЕЙСТВОВАТЬ С БЭКЛОГОМ
    }
  ...
  static async addTasks(tasks, department) {
    ...
    tasks.forEach((task, i) => { //Для каждой задачи каждой колонки каждого подразделения
      ...
      divTask.setAttribute('data-task-id', task.id);
      ...
      //тут сделать switch-функцию для отрисовки стопперов в зависимости от стадии задачи

      if (task.status == 2) {

        let divStopper = document.createElement('div'); //Ответственный
        divStopper.classList.add('task-stopper');
        divStopper.classList.add('cursor-initial');
        divStopper.setAttribute('id', "stopper-" + task.id);


        divStopper.innerHTML = Elements.divStopperStop;
        // divStopper.onclick = Click.stopperClick.bind(this, taskId, divStopper, task);
        console.log(Elements.stopper);
